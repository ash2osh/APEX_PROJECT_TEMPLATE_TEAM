...
The persistent qualification runner treats missing declarations, missing
fixtures and missing SQL/browser adapters as qualification failures. It never
converts an unavailable check into a pass. The template has no shipped
application yet, so qualification reports an explicit zero-app coverage result
until an application and its qualified adapters are added.
