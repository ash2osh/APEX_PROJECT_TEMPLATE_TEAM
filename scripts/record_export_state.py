...
def read_state(path: Path) -> tuple[str | None, datetime]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if before != after:
        print(
            ...
            f"(before: {before or 'NOT_FOUND'}, after: {after or 'NOT_FOUND'}). "
            "Wait for Builder edits to stop, then export again.",
            file=sys.stderr,
        )
        return 1

    if (before and datetime.fromisoformat(before) == before_observed_at) or (
        after and datetime.fromisoformat(after) == after_observed_at
    ):
        print(
