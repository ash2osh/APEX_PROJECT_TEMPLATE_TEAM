...
`TEAM_RELEASE` binds a version to one archive digest. A repeated cut of the same ledger/app source is deterministic; changing the cut or app inputs under an already-used version is refused. Schema releases contain the ordered ledger events and stored migration bundles. App releases contain one captured app, its required migrations at the schema cut, checks and master contract digests. Both release kinds first compare the live development schema with the frontier the ledger last accepted and refuse on drift, so an app cannot ship depending on unrecorded DDL. `run-release-test` holds each released app's mutex from a recapture that must equal the archived app bytes until qualification finishes, so an overlapping release test cannot swap the app under the checks; it also holds the migration mutex: an app release rechecks its required migrations under it, and a schema release requires the test history to still be exactly at the release cut, so no migration can land or be undone while the checks run. Replay markers written into test history are bound to a prefix of the archive's ledger digest. Release tests validate masters against the contract packaged in the archive, never the operator checkout's `targets/masters.json`. If SQLcl loses the outcome of the `TEAM_RELEASE` write, the builder keeps the archive and reports its path and digest; check `TEAM_RELEASE` for that version before building it again, because an app rebuild will not reproduce the same digest. `apply-release` re-reads the target's live metadata history and refuses a `--history` file that no longer matches it. `verify-release` remains offline and can read existing format 2 handoffs as well as format 3 archives.
