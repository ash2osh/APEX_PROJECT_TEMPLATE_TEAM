...
2. `DEVELOPER_NAME` for the DEV publish tag: uppercase letters, digits, or
   underscores, beginning with a letter and at most 30 characters.
3. Environment: `development`, `test`, `staging`, or `production`.
4. One or more positive numeric APEX application IDs as strict CSV. There is
   no separate directory name to collect: each export mirror is named by its id.
5. Tables schema, object-name prefixes, SQLcl saved-connection name, and
