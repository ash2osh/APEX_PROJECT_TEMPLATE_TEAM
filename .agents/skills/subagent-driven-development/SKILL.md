...
digraph process {
    ...
    "Setup: branch, ledger check, read plan, pre-flight review" [shape=box];
    ...
    "Setup: branch, ledger check, read plan, pre-flight review" -> "Dispatch implementer subagent (./implementer-prompt.md)";
...
switch branches when that would overwrite or strand user work. Since all
implementers share this checkout and branch, dispatch only tasks whose edits
do not overlap, and serialize any commit or index operations.
