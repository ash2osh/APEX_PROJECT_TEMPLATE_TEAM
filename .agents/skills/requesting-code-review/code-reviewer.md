...
Subagent (general-purpose):
  ...
  prompt: |
    ...
    **Head:** [HEAD_SHA]
    ...
    Your review is read-only on this checkout. Do not mutate the working tree, the index, HEAD, or branch state in any way. Use tools like `git show`, `git diff`, and `git log` to inspect history. To inspect another revision, read its files with `git show <revision>:<path>`; never switch branches or create a worktree for review.
